...
const joinTwoPathsAsRelative = p1 => p2 => _path2.default.join('./', p1, '..', p2);
...
async function resolveJSONSchemaAsync(filePath) {
  const fileContentAsHashMap = _ramda2.default.compose(readFileAsyncAsKeyvalue, joinTwoPathsAsRelative(filePath));
  ...
  for (let property in properties) {
    if (_ramda2.default.has('$ref')(properties[property])) {
      const pathKeyForHash = joinTwoPathsAsRelative(filePath)(properties[property]['$ref']);
